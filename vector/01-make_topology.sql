create table nodes ( nodeid int not null primary key, x numeric(10,6) not null, y numeric(10,6) not null, z float, edgefid int);
...
create view  v_edges AS ( SELECT e.edgeid, n1.x startx, n1.y starty, n2.x endx, n2.y endy, 'LINESTRING( ' || n1.x || ' ' || n1.y || ', ' || n2.x || ' ' || n2.y || ' ),4326' wkt FROM edges e, nodes n1, nodes n2 WHERE e.start_node_id = n1.nodeid AND e.end_node_id = n2.nodeid);
